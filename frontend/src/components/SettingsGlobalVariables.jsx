...
export default function SettingsGlobalVariables({ onAuthError }) {
  const { confirm } = useNotificationDialog();
  const [variables, setVariables] = useState([]);
  const [prefix, setPrefix] = useState(GLOBAL_PREFIX);
  ...
  const loadVariables = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await apiRequest("/api/settings/global-variables");
      const list = Array.isArray(data?.variables) ? data.variables : [];
      setVariables(list);
      setPrefix(data?.prefix || GLOBAL_PREFIX);
    } catch (err) {
  ...
  const handleDelete = async (variable) => {
    ...
    const confirmed = await confirm({
      ...
      message: "This will remove the global variable for all scripts.",
      tone: "danger",
    ...
    setIsSaving(true);
    try {
      await apiRequest(`/api/settings/global-variables/${encodeURIComponent(variable.id)}`, {
        method: "DELETE",
      });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingId) {
        await apiRequest(`/api/settings/global-variables/${encodeURIComponent(editingId)}`, {
          method: "PUT",
          body: payload,
        });
      } else {
        await apiRequest("/api/settings/global-variables", {
          method: "POST",
          body: payload,
        });
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-[color:var(--color-text-strong)]">
            Global Variables
          </h3>
          <p className="text-sm text-[color:var(--color-text-muted)]">
            Define environment variables that are injected into every script run.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        >
          No global variables configured yet.
        </div>
      ) : (
